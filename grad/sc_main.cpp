...
int sc_main(int argc, char *argv[])
{
	if(argc != 4)
	{
		...
		printf("Usage : %s image_in  image_out seuil\n", argv[0]);
		return 1;
	}
    // Recuperer l'argeument du seuil
    seuil = atoi(argv[3]);
    ...
    pixelCol = EnteteImageIn.NbColonne();
    ...
    gv1D1.blank(blank);
    gv1D1.clk(pckout);
    gv1D1.pixel_in(pixel_in);
    gv1D1.pixel_out(pixel_out);
    gv1D1.hpix_out(hpix_out);
    gv1D1.bk_out(bk_out);
